...
function UICreateScrollFrame(name, parent, template)
	...
	local function CreateScrollBar(parent, horizontal)		
		...
		bar:SetScript("OnValueChanged", function(self, arg1)
			self:UpdateButtonStatus(arg1);
			local parent = self:GetParent():GetParent();
			if parent and type(parent.OnScrollBarValueChanged) == "function" then
				parent:OnScrollBarValueChanged(self:GetParent(), arg1);
			end			
		end);	

		bar:SetScript("OnShow", function(self) self:OnScrollBarUpdated(); end);
		bar:SetScript("OnHide", function(self) self:OnScrollBarUpdated(); end);
		bar:SetScript("OnSizeChanged", function(self) self:OnScrollBarUpdated(); end);
		...
		frame:SetScript("OnMouseWheel", function(self) self._bar:SetValue(self._bar:GetValue() - arg1 * self._bar:GetValueStep() * 5); end);
	...
	scrollFrame:SetScript("OnSizeChanged", function(self)	
		...
		self:UpdateScrollBarsRange();
	end);

	scrollFrame:SetScript("OnShow", function(self) self:UpdateScrollBarsRange(); end);

	scrollFrame:SetScript("OnMouseWheel", function(self)
		local bar = self._vBar;
		if bar:IsShown() then
			bar:SetValue(bar:GetValue() - arg1 * bar:GetValueStep() * 5);
		end
	end);

	scrollFrame:SetScript("OnScrollRangeChanged", function(self, arg1, arg2) self:UpdateScrollBarsRange(arg1, arg2); end);

	scrollFrame:SetScript("OnVerticalScroll", function(self, arg1)
		local handler = self:GetParent():GetScript("OnVerticalScroll");
		if type(handler) == "function" then
			handler(self:GetParent(), arg1);
		end
	end);
	
	scrollFrame:SetScript("OnHorizontalScroll", function(self, arg1)
		local handler = self:GetParent():GetScript("OnHorizontalScroll");
		if type(handler) == "function" then
			handler(self:GetParent(), arg1);
		end
	end);

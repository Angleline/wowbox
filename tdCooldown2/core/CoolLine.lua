...
function CoolLine_ADDON_LOADED(a1)
	if (self.ADDON_LOADED) then return end
...
local CoolLine_PlayerLogin = CoolLine.PLAYER_LOGIN;
...
do  -- cache spells that have a cooldown
	local GetSpellBookItemName, GetSpellBookItemInfo, GetSpellBaseCooldown, GetSpellCharges, IsTalentSpell = GetSpellBookItemName, GetSpellBookItemInfo, GetSpellBaseCooldown, GetSpellCharges, IsTalentSpell
...
function ShowOptions(a1)
	...
	if not CoolLineDD then
		...
		local function SetColor(a1)
			local dbc = db[LIB_UIDROPDOWNMENU_MENU_VALUE]
		...
		local function AddButton(lvl, text, keepshown)
			...
			Lib_UIDropDownMenu_AddButton(info, lvl)
			wipe(info)
		end
		...
		CoolLineDD.initialize = function(self, lvl)
			...
			elseif lvl and lvl > 1 then
				local sub = LIB_UIDROPDOWNMENU_MENU_VALUE
...
-- Added by dugu
do	
	function CoolLine_Toggle(switch)
		if (switch) then		
			CoolLine_ADDON_LOADED("CoolLine");
			if IsLoggedIn() then
				CoolLine_PlayerLogin(self);
			else
				CoolLine:RegisterEvent("PLAYER_LOGIN");
			end
			CoolLine:Show();
			enable = true;
		else
	...
	function CoolLine_ToggleLock(switch)
		if (switch) then
			CoolLine.unlock = true;	
		else
			CoolLine.unlock = nil;

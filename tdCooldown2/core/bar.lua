...
function tCD:GetSpellID(spell, rank)
	...
	local i, s, r = 1;
	local spellnum = dwGetSpellNum();
	while spell do
		if (i > spellnum) then 
			break
		end

		s = GetSpellBookItemName(i, "spell")
...
function tCD:CreateCenterAnchorFrame()
	if (not self.anchor) then
		...
		self.anchor:SetWidth(140);
		self.anchor:SetHeight(140);
		...
		self.anchor.icon:SetTexture("Interface\\ICONS\\Ability_Ambush");

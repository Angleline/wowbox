...
frame:SetPoint("LEFT", UIParent, "LEFT", 80, 0) --init pos by eui.cc
frame:SetWidth(180)
frame:SetHeight(15)
...
do
	...
	frame:RegisterEvent("ADDON_LOADED")  --by eui.cc
end
...
frame:SetScript("OnEvent", function(f, event, ...)
	if (event =="ADDON_LOADED") and select(1,...) == "LegionInvasionTimer" then
		f:UnregisterEvent("ADDON_LOADED") --by eui.cc
	end
	...
	header:SetText(L.name) --by eui.cc
	f.header = header

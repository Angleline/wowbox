...
core:AddCallback('Initialize', nil, function(self, ...)
	...
	tab.showPlayerBuff = false --by eui.cc
	tab.showPlayerDebuff = false --by eui.cc
...
local function ScanAuras(self, nameplate, token)
	...
	while token and spellID do
		name, icon, count, debuffType, duration, expirationTime, caster, _, _, spellID = UnitAura(token, index, filter)
		if spellID then

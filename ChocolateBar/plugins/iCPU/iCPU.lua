...
local db = {
	...
	PluginShowLatencyHome = false,
	PluginShowLatency = true,
	PluginShowCPU = true,
	TooltipShowFramerate = true,
	ToolTipShowLatencyHome = false,
...
local function format_memory(value, state, space)
	...
	if( value > 1024 ) then
		value = ("%."..db.DecimalDigits.."f"..(space and "" or " ").."|cffff8100%s|r"):format((value / 1024), L["mb"]);
	else
		value = ("%."..db.DecimalDigits.."f"..(space and "" or " ")..COLOR_GOLD):format(value, L["kb"]);
	end
...
local function format_cpu(value, state, space)	
	if( state ) then
		value = ("|cff00ffff%."..db.DecimalDigits.."f|r"..(space and "" or " ")..COLOR_GOLD):format(value, L["ms"]);
	else
		value = ("|cffee33ff%."..db.DecimalDigits.."f|r"..(space and "" or " ")..COLOR_GOLD):format(value, L["ms"]);
	end
...
local function format_kbs(value)
	return ("%."..db.DecimalDigits.."f "..COLOR_GOLD):format(value, L["kb/s"]);
end
...
local function format_fps(value, space)
	return ("|cff%s%."..db.DecimalDigits.."f|r"..(space and "" or " ")..COLOR_GOLD):format(LibCrayon:GetThresholdHexColor(value, hugeFramerate), value, L["fps"]);
end
...
iCPU.ldb.OnEnter = function(anchor)
	...
	if( db.TooltipUseScroller ) then
		tip:UpdateScrolling(db.TooltipMaxSize);
	end
...
do
	...
	--	self.db = LibStub("AceDB-3.0"):New("iCPUDB", self:CreateDB(), "Default").profile;
		...
		local name, cname, title, enabled, reason, _;
		local Iter = 1;
		
		for i = 1, _G.GetNumAddOns() do
			name, cname, _, enabled, _, reason, _ = _G.GetAddOnInfo(i);
...
function iCPU:UpdateBroker()
	if( not db.PluginShowMemory and not db.PluginShowFramerate and not db.PluginShowLatency
			and not (isProfiling and not db.PluginShowCPU)) then
		self.ldb.text = "|cffffff00"..AddonName.."|r";
	else		
		self.ldb.text =
			(isProfiling and db.PluginShowCPU and format_cpu(TotalCPU, true, true).." " or "")..
			(db.PluginShowMemory and format_memory(TotalMemory, nil, true).." " or "")..
			(db.PluginShowFramerate and format_fps(iFramerate, true).." " or "")..
			(db.PluginShowLatencyHome and format_latency(iLatencyHome, true).." " or "")..
			(db.PluginShowLatency and format_latency(iLatencyWorld, true).." " or "")
		;
	end
end
...
function iCPU:UpdateTooltip(tip)
	...
	local numAddons = db.DisplayNumAddons > #Mods and #Mods or db.DisplayNumAddons;
	...
	if( firstDisplay ) then
		...
		if( db.TooltipShowFramerate or db.TooltipShowLatency or db.TooltipShowStreaming ) then
			tip:AddLine(" ");
			
			if( db.TooltipShowFramerate ) then
				line = tip:AddLine(" ");
				tip:SetCell(line, 1, (COLOR_GOLD):format(L["Framerate"]), nil, "LEFT", -2);
			end
			
			if( db.TooltipShowLatencyHome ) then
				line = tip:AddLine(" ");
				tip:SetCell(line, 1, (COLOR_GOLD):format(L["Latency Home"]), nil, "LEFT", -2);
			end
			
			if( db.TooltipShowLatency ) then
				line = tip:AddLine(" ");
				tip:SetCell(line, 1, (COLOR_GOLD):format(L["Latency World"]), nil, "LEFT", -2);
			end
			
			if( db.TooltipShowStreaming ) then
				line = tip:AddLine(" ");
	...
	if( db.TooltipUseScroller and tip:IsVisible() ) then
		tip:UpdateScrolling(db.TooltipMaxSize);
	end
end

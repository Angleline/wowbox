if IsAddOnLoaded("Easy_Obliterate") then return; end
...
mainFrame.totalText:SetText("总计: 55")
...
mainFrame.autoLootCheck:SetScript('OnClick', function(self)
    if GetCVar('autoLootDefault') == '1' then
        DEFAULT_CHAT_FRAME:AddMessage('Easy Obliterate: 自动拾取不能被禁用，因为你系统的自动拾取是开启的.')
...
local function showTooltip(self)
    if self.itemRef then
        if not eligibleItems[self.itemRef].ignore then
            ...
            GameTooltip:AddLine('左击添加到分解页.', 1, 1, 1);
            GameTooltip:AddLine('右击忽略/取消忽略此物品.', 1, 1, 1);
            GameTooltip:Show()
        else
            GameTooltip:SetOwner(self, "ANCHOR_MOUSE");
            GameTooltip:SetText(eligibleItems[self.itemRef].itemName)
            GameTooltip:AddLine('|cFFFFFFFF此物品在你的忽略列表.|r')           
            GameTooltip:Show()        
        end
    end
end
...
local function addItemToForge(self, button)
    if not ashLooted then
        DEFAULT_CHAT_FRAME:AddMessage('Easy Obliterate: 关闭拾取窗口.')
        CloseLoot()
        return
    end
...
local function updateText()
   mainFrame.totalText:SetText("总计: "..#eligibleItems)
    ...
    elseif event == 'OBLITERUM_FORGE_PENDING_ITEM_CHANGED' then
        if C_TradeSkillUI.GetPendingObliterateItemLink() then
            if not inIgnoreList(C_TradeSkillUI.GetPendingObliterateItemLink()) then
                ...
                C_TradeSkillUI.ClearPendingObliterateItem()
                DEFAULT_CHAT_FRAME:AddMessage('Easy Obliterate: 此物品在你的忽略列表!')
            end
        else
            hideSelection()
        end

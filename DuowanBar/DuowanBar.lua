...
D.options = {
	...
	args = {
		...
		["Scale"] = {
			...
			get = function()
				local bar = curBar;
				return D:GetCVar(bar, "scale");
			end,
			set = function(v)
				local bar = curBar;
				D:SetCVar(bar, "scale", v);
				dwSetScale(bar, v);
			end,
			min = 0.5,
			max = 2,
...
function D:ADDON_LOADED(event, name, ...)
	if (name == "DuowanBar") then
...
function D:SaveButtonInfo(button, data)
	...
	local curTalent = GetActiveSpecGroup() or 1;
...
function D:CreateActionBar(index, unit)
	...
	if (not bar) then	
		...
		bar.tab:SetScript("OnMouseDown", function(self, button)
			if (not D.db[name] or not D.db[name]["locked"] and button ~= "RightButton") then
				self.ismoving = true;
				bar:StartMoving();
				SimpleSticky:StartMoving(bar, StickyBarList, 0, 0, 0, 0);
			end
		end);
		bar.tab:SetScript("OnMouseUp", function(self, button)
			if (self.ismoving) then
				self.ismoving = false;				
				bar:StopMovingOrSizing();	
				SimpleSticky:StopMoving(bar);
				local xOffset, yOffset = bar:GetLeft(), bar:GetTop();
				--local point, relativeTo, relativePoint, xOffset, yOffset = bar:GetPoint();
				D:SaveBarInfo(bar, {region={"TOPLEFT", "BOTTOMLEFT", xOffset, yOffset}});
			end
		end);
...
function D:UpdateBar(bar)
	...
	if (db) then
		-- 定位 缩放
		db.region = db.region or {"CENTER", "CENTER", 0, 60};
		db.scale = db.scale or 1.0;	
		dwSetScale(bar, db.scale);
		bar:ClearAllPoints();
		...
		-- 排列
		db.arrange = db.arrange or VERTICAL;		

...
function RareScanner:Toggle(toggle)
	if (toggle or toggle == 1) then
		...
		scanner_button:UnregisterEvent("VIGNETTE_MINIMAP_UPDATED")

		-- Out of combat events
		scanner_button:UnregisterEvent("PLAYER_REGEN_ENABLED")

		-- Unit deaths
		scanner_button:UnregisterEvent("COMBAT_LOG_EVENT_UNFILTERED")
		scanner_button:UnregisterEvent("PLAYER_TARGET_CHANGED")

		-- Items events
		scanner_button:UnregisterEvent("GET_ITEM_INFO_RECEIVED")
		scanner_button:UnregisterEvent("LOOT_OPENED")

		-- Avoid addon on cinematics
		scanner_button:UnregisterEvent("CINEMATIC_START")

		-- Chat messages
		scanner_button:UnregisterEvent("CHAT_MSG_MONSTER_YELL")
		scanner_button:UnregisterEvent("CHAT_MSG_MONSTER_EMOTE")
	end
end
scanner_button:RegisterEvent("ADDON_LOADED")

-- Captures all events
scanner_button:SetScript("OnEvent", function(self, event, ...)
	if (event == "ADDON_LOADED") then
		local arg1, arg2 = ...;
		if arg1 == "RareScanner" then
			local toggle = dwRawGetCVar("MiniMapMod", "NPCScanEnable", 1);
			RareScanner:Toggle(toggle)

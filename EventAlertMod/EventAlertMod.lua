...
EA_Config2 = 	{
		...
		--單一技能冷卻完成即移除(true:要移�?false:不移�?
		SCD_RemoveWhenCooldown	= true,		
		
		--剩餘多少秒開始使用小數點一位顯�?使用0則完全不會有小數�?
		UseFloatSec				= 1,
		...
		--BUFF/DEBUFF 的Value 值大於等於指定數值才會顯�?
...
--	end
	self:RegisterEvent("ADDON_LOADED", EventAlert_ADDON_LOADED)
...
function EventAlert_InitArrayConfig()
	...
	if EA_Config.IconSize == nil then EA_Config.IconSize = 45 end;
		...
		if (eaf ~= nil) then
			if (iComboPoint > 0) then
				...
				if ComboPointMax == 6 then		--7.0盜賊天賦:精明戰略
					...
					GlowComboPoint = 5			--7.0非精明戰略下, 最大值不�?就是8, 一律以5做為高亮�?
				end
...
local function checkBuff(sUnitType, sSpellName)
	for i = 1, 40 do
		local sCurrSpellName, arg1,iStack, arg2, arg3, iExpireTime = UnitBuff(sUnitType, 1)
		if sCurrSpellName == sSpellName then
			return sCurrSpellName, arg1,iStack, arg2, arg3, iExpireTime
		end
	end
...
local function checkDebuff(sUnitType, sSpellName)
	for i = 1, 40 do
		local sCurrSpellName, arg1,iStack, arg2, arg3, iExpireTime = UnitDebuff(sUnitType, 1)
		if sCurrSpellName == sSpellName then
			return sCurrSpellName, arg1,iStack, arg2, arg3, iExpireTime
		end
	end
...
function EventAlert_GroupFrameCheck_OnEvent(self, event, ...)
	...
	if (not fShowResult) then
		...
		--	EventAlert_UNIT_POWER_UPDATE(self, event, ...)
			-- SPEC EVENT FIRED, To check all INDEXD-EVENTCFG about this frame(by GroupIndex).
				...
				if (sUnitType == SubCheck.UnitType or SubCheck.UnitType == "all") then -- "player"
					...
					if (fShowResult) then
						...
						if (SubCheck.CheckAuraExist ~= nil) then
							...
							local sCurrSpellName, _,iStack, _, _, iExpireTime = checkBuff(sUnitType, sSpellName);
							if sCurrSpellName ~= nil then
								fShowResult = true;
							else
								sCurrSpellName, _,iStack, _, _, iExpireTime = checkDebuff(sUnitType, sSpellName);
								if sCurrSpellName ~= nil then
									fShowResult = true;
								end
							end
						...
						if (SubCheck.CheckAuraNotExist ~= nil) then
							...
							local sCurrSpellName = checkBuff(sUnitType, sSpellName);
							if sCurrSpellName == nil then
								sCurrSpellName = checkDebuff(sUnitType, sSpellName);
								if sCurrSpellName == nil then
									fShowResult = true;
								end
							end
...
function EventAlert_Toggle(toggle)
	if toggle then
		...
		for k, v in pairs(EA_EventList) do
		--	print(k)
			EA_Main_Frame:RegisterEvent(k,v)
		end
		EA_Main_Frame:Show();
		...
		-- To register events from 'EA_EventList' function array.
		for k, v in pairs(EA_EventList) do
			EA_Main_Frame:UnregisterEvent(k,v)
		end
		EA_Main_Frame:Hide();
		EventAlert_Enable = false;
	end

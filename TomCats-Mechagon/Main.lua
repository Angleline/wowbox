...
function initializeQuests()
    ...
    for k, v in pairs(D["Creatures"].records) do
        ...
        if (questID) then
            ...
            local button = CreateFrame("BUTTON", "TomCats-MechagonRareButton"..k, UIParent, "TomCats-MechagonRareButtonTemplate")
            quest.label = label
            quest.button = button
            if (namesLoaded) then
...
function addon:toggleTourWindow()
    ...
    elseif (tourWindow) then
        ...
        PlaySound(799)
        ----------------------------------
        newTourWindow = CreateFrame("Frame", "TomCats-MechagonTourWindow", UIParent, "TomCats-MechagonTourWindowTemplate")
        --newTourWindow:Show()
        _G["TomCats-MechagonTourWindowPortrait"]:SetTexture("Interface\\AddOns\\" .. addon.name .. "\\images\\" .. addon.params["Minimap Icon"]);
        _G["TomCats-MechagonTourWindowPortrait"]:SetTexCoord(0, 1, 0, 1);
        _G["TomCats-MechagonTourWindowPortraitBackground"]:SetTexture("Interface\\CHARACTERFRAME\\TempPortraitAlphaMaskSmall");
        _G["TomCats-MechagonTourWindowPortraitBackground"]:SetVertexColor(118/255,18/255,20/255,1);
        _G["TomCats-MechagonTourWindowTitleText"]:SetText("|cFFFFFFFF" .. L["TomCat's Tours"]);
        _G["TomCats-MechagonTourWindowTitle2Text"]:SetText(L[addon.params["Title Line 1"]] .. "\n(" .. L[addon.params["Title Line 2"]] .. ")");
        ...
        for i = 1, #buttons, 1 do
            buttons[i]:SetParent(_G["TomCats-MechagonTourWindowRaresScrollFrameScrollChild"])
            if i > 1 then
                buttons[i]:SetPoint("TOPLEFT", buttons[i-1]:GetName(), "BOTTOMLEFT", 0, 0)
            else
                buttons[i]:SetPoint("TOPLEFT", _G["TomCats-MechagonTourWindowRaresScrollFrameScrollChild"], "TOPLEFT", 0, 0)
            end
            buttons[i]:Show()
        end

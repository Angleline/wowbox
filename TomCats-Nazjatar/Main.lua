...
function addon:toggleTourWindow()
    ...
    elseif (tourWindow) then
        ...
        PlaySound(799)
        ----------------------------------
        newTourWindow = CreateFrame("Frame", "TomCats-NazjatarTourWindow", UIParent, "TomCats-NazjatarTourWindowTemplate")
        --newTourWindow:Show()
        _G["TomCats-NazjatarTourWindowPortrait"]:SetTexture("Interface\\AddOns\\" .. addon.name .. "\\images\\" .. addon.params["Minimap Icon"]);
        _G["TomCats-NazjatarTourWindowPortrait"]:SetTexCoord(0, 1, 0, 1);
        _G["TomCats-NazjatarTourWindowPortraitBackground"]:SetTexture("Interface\\CHARACTERFRAME\\TempPortraitAlphaMaskSmall");
        _G["TomCats-NazjatarTourWindowPortraitBackground"]:SetVertexColor(118/255,18/255,20/255,1);
        _G["TomCats-NazjatarTourWindowTitleText"]:SetText("|cFFFFFFFF" .. L["TomCat's Tours"]);
        _G["TomCats-NazjatarTourWindowTitle2Text"]:SetText(L[addon.params["Title Line 1"]] .. "\n(" .. L[addon.params["Title Line 2"]] .. ")");
        ...
        for i = 1, #buttons, 1 do
            buttons[i]:SetParent(_G["TomCats-NazjatarTourWindowRaresScrollFrameScrollChild"])
            if i > 1 then
                buttons[i]:SetPoint("TOPLEFT", buttons[i-1]:GetName(), "BOTTOMLEFT", 0, 0)
            else
                buttons[i]:SetPoint("TOPLEFT", _G["TomCats-NazjatarTourWindowRaresScrollFrameScrollChild"], "TOPLEFT", 0, 0)
            end
            buttons[i]:Show()
        end
    ...
    TomCats:Register(
        {
            slashCommands = {
                {
                    command = "NAZJATAR TOGGLE",
                    desc = "Toggle Rares of Nazjatar Window",

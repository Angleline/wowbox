...
A.spellsDB = {
	...
	["ROGUE"] = {
		...
		[2] = 114018,		-- 暗影之舞？
		-- 毒药(itemID可能获取不到信息...)
		[2] = 2823,--{t = "p", name = L["致命毒药"]},
	--	[4] = 157584,--{t = "p", name = L["速效药膏"]},?
		[3] = 8679,--{t = "p", name = L["致伤药膏"]},
		[4] = 3408,--{t = "p", name = L["减速药膏"]},	
		[5] = 108211,--{t = "p", name = L["吸血药膏"]},
		[6] = 200802,--苦痛药膏
	},
	["MAGE"] = {
		[1] = {t = "d", name = L["埃索达"]},
			...
			if (type(spellID) == "table") then				
				...
				elseif (spellID.t and spellID.t == "d") then	-- 传送门
					...
					if spellID.name == L["达拉然"] then
						check1, check2 = self:CheckSpell(L["传送门: "] .. spellID.name), self:CheckSpell(L["传送: "] .. L["达拉然 - 破碎群岛"]);
					else
			...
			if (button.spellName ~= name) then
				if (InCombatLockdown()) then
					...
					elseif (type == "teleport") then
						...
						--else
						if name == L["达拉然"] then
							button:SetAttribute("spell1", L["传送: "] .. L["达拉然 - 破碎群岛"]);
...
function A:UIParent_ManageFramePositions()
	...
	if ( StatusTrackingBarManager.SingleBarLarge:IsShown() or StatusTrackingBarManager.SingleBarSmall:IsShown() ) then --ReputationWatchBar miss?
		YOffset = YOffset + 10;
	end

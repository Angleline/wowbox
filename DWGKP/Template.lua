...
local RADIO_MAP_CLASSES = {
	[1] = {"Mage", "Warlock", "Priest", "Druid", "Paladin", "Rogue", "Shaman", "Hunter", "Warrior", "DeathKnight", "Monk"},
...
local function DWGKP_GetClasses(self)
	...
	for k, v in ipairs(RADIO_MAP_CLASSES[1]) do
		if (_G["DWGKPDropdownBox" .. v .. "Button"]:GetChecked() == true) then
...
function ToggleDropdownBox(self)
	...
	if (DWGKPDropdownBox:IsVisible() and DWGKPDropdownBox:GetAlpha() > 0) then
		...
		for k, v in ipairs(RADIO_MAP_CLASSES[1]) do
			_G["DWGKPDropdownBox" .. v .. "Button"]:SetChecked(false);
		end
		for k, v in ipairs(DWGKP_ClothGroup) do
			_G["DWGKPDropdownBox" .. v ]:SetChecked(false);
		end
		
		_G["DWGKPDropdownBox" .. self.RadioGroup]:SetChecked(true);
...
function DWGKPDropdownBox_Hide()
	DWGKPDropdownBox.showTimer = nil;
	DWGKPDropdownBox.isCounting = nil;
	dwFadeOut(DWGKPDropdownBox, 0.2, 1, 0);
end

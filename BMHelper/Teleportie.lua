...
function UnitBuffName(unit, name)
	for i = 1, 60 do
		local spellName, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, nameplateShowPersonal, spellId, canApplyAura, isBossDebuff, nameplateShowAll, timeMod, value1, value2, value3 = UnitBuff(unit, i)
...
local function CheckPortalBuffFade()
	if not UnitBuffName("player", BUFF_NAME) then
		f:Hide()
		wipe(Portal)
...
local function CheckPortalBuffApply()
	if UnitBuffName("player", BUFF_NAME) then

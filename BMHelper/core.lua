...
function bmh:UNIT_POWER_UPDATE()
	update()
end
...
bmh:RegisterEvent("UNIT_POWER_UPDATE")
...
if not GetSpellInfo(id) then print(id); return; end
...
-- UpdateAura
-----------------------------------
local function getUnitInfo(aura_name)
	local name, icon, count, dispelType, duration, expires, caster, isStealable, shouldConsolidate, spID
	for i = 1, 40 do
		name, icon, count, dispelType, duration, expires, caster, isStealable, shouldConsolidate, spID = UnitAura("player", i)
		if(aura_name == name) then return name, icon, count, dispelType, duration, expires, caster, isStealable, shouldConsolidate, spID; end
...
local function createbuff(i, id, auratype, show, tc, tbl, ctt, gw, growtime, growcount)
	...
	local name, icon, count, dispelType, duration, expires, caster, isStealable, shouldConsolidate, spID = getUnitInfo(aura_name)
...
local function createcooldown(i, id, show, cd, gw, growtime)
	...
	if start and duration then
		...
		if not ((value > 0) and duration > 2) then
				...
				if i:GetNormalTexture() then i:GetNormalTexture():SetDesaturated(false) end
		else --item is on cooldown show time
				if not show then
					i:SetAlpha(0)
				end
				if i:GetNormalTexture() then i:GetNormalTexture():SetDesaturated(true) end   
		end
	end
		-- cd
	if cd then
...
local function checkaura(id,auratype,atime,acount)
	local aura_name, aura_rank, aura_icon = GetSpellInfo(id)
	local name, icon, count, dispelType, duration, expires, caster, isStealable, shouldConsolidate, spID = getUnitInfo(aura_name)--UnitAura("player", aura_name, aura_rank, auratype)
...
createbutton(icon1, 195630, 35, true)
...
createbutton(icon13, 100780, 25, true)
...
bmhaura:SetScript("OnEvent", function(self,event)
	if event == "UNIT_AURA" or event == "UNIT_HEALTH" or event == "UNIT_POWER_UPDATE" or event == "PLAYER_REGEN_DISABLED" then
		update()
		update2()
		update3()
...
bmhaura:RegisterEvent("UNIT_POWER_UPDATE")

...
function DWChatIconFrame:OnEnable()	
	for i=1, 10 do
		_G["ChatFrame" .. i .. "EditBox"]:SetScript("OnTextChanged",function(self) 
...
local fmtstring = format("\124T%%s:%d\124t",max(floor(select(2,SELECTED_CHAT_FRAME:GetFont())),16))

local function myChatFilter(self, event, msg, ...)
	for i = 9, #DWC_IconTable do
		if msg:find(DWC_IconTable[i][1]) then
			msg = msg:gsub(DWC_IconTable[i][1],format(fmtstring,DWC_IconTable[i][2]),1)
			break
		end
	end
	return false, msg, ...
...
function HandleBubbleEmote(frame, fontstring)
	...
	for i = 9, #DWC_IconTable do
		if text:find(DWC_IconTable[i][1]) then
			text = text:gsub(DWC_IconTable[i][1],format(fmtstring,DWC_IconTable[i][2]),1)
			break
		end
	end
...
local function CheckBubbles()
	for i=1,WorldFrame:GetNumChildren() do
		...
		local b = pcall(v, GetBackdrop)
		if b then b = v:GetBackdrop() end

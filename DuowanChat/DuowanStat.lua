...
local L_ArtifactForgeLevel = "神器总等级"
...
function D:UpdateStat()
	...
	if (self.data["LV"] > 80) then
		...
		self.data["MST"] = format("%d%%", mastery + 0.5);
	end
...
--	self.data["SPI"] = UnitStat("player", 5);					--精神
	...
	self.data["SHASTE"] = GetHaste() + 0.5;				--急速等级
...
function D:GenerateStatText(detail)
	...
	local Alv = self:GetArtifactForgeLevel();
	if Alv then
		text = text .. "，" .. L_ArtifactForgeLevel..": "..Alv;
	end
	...
	if (detail) then
		...
		if (UnitPowerType("player") == 0) then
			text = text..("，%0.2f%s，%d%s"):format(self.data["HP"]/10000, '万'..L["HP"], self.data["MP"], L["MP"]);
		else
			text = text..("，%0.2f%s"):format(self.data["HP"]/10000, '万'..L["HP"]);
		end
		...
		elseif self.data["CLASS_EN"] == "DEMONHUNTER" then
			if (specIndex == 2) then
				text = text..self:GetTankText();
			else
				text = text..self:GetMeleeText();
...
--	text = text..("，%d%s"):format(self.data["SHIT"], L["HIT"]);
	...
	if (self.data["MST"]) then
		text = text..("，%s%s"):format(self.data["MST"], L["MST"]);
	end
	if self.data["PvPSET"] then
...
function D:GetHealText()
	local text = "";
	text = text..self:GetPvPSpecText();
	text = text..(", %d%s"):format(self.data["STA"], L["STA"]);
	text = text..(", %d%s"):format(self.data["INT"], L["INT"]);
...
function D:GetMeleeText()
	...
	text = text..self:GetPvPSpecText();
	text = text..(", %d%s"):format(self.data["STA"], L["STA"]);
	if self.data["CLASS_EN"] ~= "WARRIOR" and self.data["CLASS_EN"] ~= "ROGUE" and self.data["CLASS_EN"] ~= "DEATHKNIGHT" and self.data["CLASS_EN"] ~= "DEMONHUNTER" then
		text = text..(", %d%s"):format(self.data["INT"], L["INT"]);
	end
...
function D:GetRangedText()
	local text = "";
	text = text..self:GetPvPSpecText();
	text = text..(", %d%s"):format(self.data["STR"], L["STR"]);
	text = text..(", %d%s"):format(self.data["AGI"], L["AGI"]);
	text = text..(", %d%s"):format(self.data["STA"], L["STA"]);
...
function D:GetTalent()	
	...
	if (UnitLevel("player") >= 10) then	
		...
		local _, specName, _, _, role = GetSpecializationInfo(index);
...
function D:GetMainTalentName()	
	if (UnitLevel("player") >= 10) then		
		...
		local id, name, description, icon, role = GetSpecializationInfo(currentSpec);
...
function D:GetArtifactForgeLevel()

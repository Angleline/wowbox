	...
	elseif event=="FRIENDLIST_UPDATE" then 
		...
		if num>0 then
			for i=1,num,1 do
				...
				if info.className and not checkInfo(info.name, info.level) then
					local classFileName=DWC_FILENAME[info.className]
					storeName(info.name,classFileName,info.level) 
				end
			end
		end 
...
for i = 1, NUM_CHAT_WINDOWS do 
	if ( i ~= 2 ) then 
		...
		f.AddMessage = function(frame, text, ...)
			for k,v in pairs(SCCN_Chan_Replace) do 
				temp = gsub(text, " "..k.."%]", v.."%]", 1);
				temp = gsub(temp, k.."%]", v.."%]", 1);
				temp = gsub(temp, k.."：", v.."：", 1);
				temp = gsub(temp, "^"..k, v, 1);
				if temp ~= text then 
					text = temp;
...
function DuowanChat:Refresh() 	
	...
	if db and db.frameposition then 
		...
		DWCChatFrame:SetPoint("TOPLEFT", "ChatFrame1ButtonFrame","BOTTOMLEFT",0,-2)
	end
end 

...
AF_Scale = 1,						--缩放比例
MAXRAM = 2160,						--内存清理
BossHealthPecentOnOff = false,		--Boss血量
BossHealthPecent = 10,				--通报上限
BossKill = false,					--最后一击
TANK_DangerHealth = false,			--TANK血量
TANK_DangerHealthPecent = 30,		--危急上限
TANK_Death = false,					--TANK阵亡
TANK_ShieldWall = true,				--盾墙
TANK_LastStand = true,				--破釜沉舟
TANK_ShieldReflection = false,		--法术反射
ScrMsg = true,						--屏幕讯息
...
RES_DispelMagic = false,				--抵抗驱散
RES_Polymorph = false,				--抵抗变形
RES_MagicBreak = false,				--抵抗反制
RES_BuffStolen = false,				--抵抗法偷
Check_Poly = false,					--被控制
PolyDispel = false,					--解除控制
Break_Poly = false,					--打破控制
Dispel_Magic = true,				--魔法驱散
BuffStolen = false,					--法术偷取
Break_Magic = true,					--魔法打断
Check_Gem = true,					--灵魂绑定
Paladin_Intervention = true,		--神圣干涉
Check_Death_AtCombatEnd = false,		--阵亡通报
...
RaidAlerter_SOUND_DATA = {
	[1]	= "Sound\\Doodad\\BellTollNightElf.ogg",
	[2]	= "Sound\\interface\\AuctionWindowOpen.ogg",
	[3]	= "Sound\\Doodad\\BellTollAlliance.ogg",
	[4]	= "Sound\\Spells\\SeepingGaseous_Fel_Nova.ogg",
	[5]	= "Sound\\Spells\\PVPFlagTaken.ogg",
};
...
function RaidAlerter_CombatLogEvent(self, ...)
	if RaidAlerter_SET.OnOff then
		-- new add 增加 otherPara1,otherPara2
		if GameVER >= 40100 then		--3.35-4.1兼容过渡
			if GameVER >= 40200 then
				local tmpone,tmptwo;
				...
				timestamp, eventtype, _, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, spellSchool, auraType, amount, otherPara1, otherPara2 = ...;
			end
			-- timestamp, eventtype, _, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, spellSchool, auraType, amount, otherPara1, otherPara2 = ...;
		else
			timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, spellSchool, auraType, amount, otherPara1, otherPara2 = ...;
		end
		local t1,t2;
...
function RaidAlerter_Raid_PARTY_Combat_Alerter()
	...
	if eventtype == "SPELL_AURA_APPLIED" then
		...
		if spellName == RAL_TEXT_SPELL_3 or spellId == 20707 then
			TempUnit1 = RaidAlerter_GUIDToUnit(dstGUID);
			if TempUnit1 then

...
function mod:SetupMap()
	...
	if mod.db.point then
		...
		Minimap:ClearAllPoints()
		Minimap:SetPoint('CENTER', MinimapCluster, 'TOP', 9, -102) --by cadcamzy
	end
	self.SetupMap = nil
end
...
function SexyMap_Toggle(switch)
	SexyMap_ToggleBorder(switch)
	SexyMap_ToggleButton(switch)
	SexyMap_ToggleZoneText(switch)
	SexyMap_ToggleClock(switch)
	SexyMap_ToggleCoordinates(switch)
end
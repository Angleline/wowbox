...
function B:AuraButton_Update(buttonName, _index, filter)
	...
	if (buff) then
		...
		local name, texture, count, debuffType, duration, expirationTime = UnitAura(unit, _index, filter);
...
local function HookBuffCaster(o, ...)
	...
	local _,  _, _, _, _, _, casterUnit = UnitAura(uid, id, f);
	local classEN, str
	if(casterUnit) then
...
local function HookTargetMounts(o, ...)
	...
	local id = select(10,UnitAura(uid, id, f))
	if id then

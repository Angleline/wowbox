...
local DefaultSound = "Sound\\Character\\PlayerExertions\\GnomeMaleFinal\\GnomeMaleMainDeathA.ogg";
...
EventFuncs = {
	VARIABLES_LOADED = function()
	  ...
	  ADDON_ACTION_BLOCKED = function(AddOn, FuncName)
		if Config.Taint and _DEBUG then
			BaudErrorFrameAdd(AddOn.." blocked from using "..FuncName,4);
		end
	  end,
	  ...
	  LUA_WARNING  = function(_, _, warningMessage)
		if _DEBUG then
			BaudErrorFrameAdd(warningMessage,4);
		end
	  end,
};

...
if(not dwStaticPopupDialogs) then dwStaticPopupDialogs = StaticPopupDialogs end
...
function dw_ShortValue(v)
	if dwGetCVar("RecountMod", "NumberValue") == "英文千位" then
		if abs(v) >= 1e9 then
	...
	elseif dwGetCVar("RecountMod", "NumberValue") == "中文万位" then
		local y = '亿';
		local w = '万';
	...
	elseif dwGetCVar("RecountMod", "NumberValue") == "英文万位" then
		if abs(v) >= 1e8 then
	...
	elseif dwGetCVar("RecountMod", "NumberValue") == "详细数值" then
...
-- Interface: 70200
-- Version: v1.0.3
-- Title: FixOrderHallMap
-- Notes: Properly zooms out from Paladin, Priest and Monk order hall to Broken Isles

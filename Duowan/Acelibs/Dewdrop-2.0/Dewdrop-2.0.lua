...
local Dewdrop, oldminor = LibStub:NewLibrary(MAJOR_VERSION, MINOR_VERSION)
local self = Dewdrop
local type = type
...
local function confirmPopup(message, func, ...)
	if not dwStaticPopupDialogs["DEWDROP20_CONFIRM_DIALOG"] then
		dwStaticPopupDialogs["DEWDROP20_CONFIRM_DIALOG"] = {}
	end
	local t = dwStaticPopupDialogs["DEWDROP20_CONFIRM_DIALOG"]
	for k in pairs(t) do
		t[k] = nil
	end
	t.text = message
	...
	Dewdrop:Close()
	dwStaticPopup_Show("DEWDROP20_CONFIRM_DIALOG")
end
...
function Dewdrop:InjectAceOptionsTable(handler, options)
	...
	if not LibStub("AceOO-2.0") or not class then
		if Rock then
		...
		-- Ace2 object
		while class and class ~= LibStub("AceOO-2.0").Class do

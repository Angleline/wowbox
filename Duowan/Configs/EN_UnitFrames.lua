if (GetLocale() == "zhCN") then
	...
	EUF_PLAYERFRAME_HPMP2			= "显示扩展生命值和魔法值"
	-- target
	...
	EUF_PARTYFRAME_SHOW_IN_RAID		= "在团队中显示小队头像";
	-- general
...
elseif (GetLocale() == "zhTW") then
	...
	EUF_PARTYFRAME_SHOW_IN_RAID		= "在團隊中顯示小隊頭像";
...
if (dwIsConfigurableAddOn("EN_UnitFrames")) then
	...
	dwRegisterCheckButton(
		...
		function (arg)			
			if (arg == 1) then
				if (dwIsAddOnLoaded("EN_UnitFrames")) then
					EUF_HPMP2Toggle(true);
				end
			else
				if (dwIsAddOnLoaded("EN_UnitFrames")) then
					EUF_HPMP2Toggle(false);
				end
			end
		end,
		1
	...
	dwRegisterCheckButton(
		...
		function (arg)			
			if (arg == 1) then
				...
				TargetFrameTextureFrameManaBarTextLeft:SetPoint("LEFT", TargetFrameTextureFrame, 99998, -8)
				TargetFrameTextureFrameManaBarTextRight:SetPoint("RIGHT", TargetFrameTextureFrame, -99998, -8)
			else
				if (dwIsAddOnLoaded("EN_UnitFrames")) then
					...
					TargetFrameTextureFrameManaBarTextLeft:ClearAllPoints()
					TargetFrameTextureFrameManaBarTextRight:ClearAllPoints()
					TargetFrameTextureFrameManaBarTextLeft:SetPoint("LEFT", TargetFrameTextureFrame, 8, -8)
					TargetFrameTextureFrameManaBarTextRight:SetPoint("RIGHT", TargetFrameTextureFrame, -110, -8)
				end
			end
		end
	);
...
--	bDefaultValue = dwRawGetCVar("EN_UnitFrames", "showInRaid", bDefaultValue);
	dwRegisterCheckButton(
		"EN_UnitFrames",
		EUF_PARTYFRAME_SHOW_IN_RAID,
		"",
		"showInRaid",
		0,
		function (arg)

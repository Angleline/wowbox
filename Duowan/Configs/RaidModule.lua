...
if (GetLocale() == "zhCN") then	
	...
	RECOUNT_SPEEDUP = "快速更新";
	RECOUNT_SPEEDUP_DESC = "加快Recount更新速度，达到每秒两次！";
	...
	NUMBER_VALUE = {"英文千位","英文万位", "中文万位", "详细数值"};
	NUMBER_VALUE_DEFAULT = "英文千位";
	NUMBER_FORMAT = "数字格式"
elseif (GetLocale() == "zhTW") then 
	...
	RAID_MOD_AngryKeystones_OPTION = "大秘境计时设置"
	RECOUNT_SPEEDUP = "快速更新";
	RECOUNT_SPEEDUP_DESC = "加快Recount更新速度，達到每秒兩次！";
...
if (dwIsConfigurableAddOn("RaidAlerter")) then
	...
	dwRegisterButton(
		...
		function()			
			if (dwIsAddOnLoaded("RaidAlerter")) then
				RaidAlerter_Option_MainFrame_Show()
			end
		end, 
		1
	);	

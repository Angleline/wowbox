...
if (GetLocale() == "zhCN") then
	...
	DUOWAN_GEARSCORELITE = "开启装备评分(GS)";
	DUOWAN_ITEMLEVEL_TITLE = "开启装备等级显示"
	GSS_TITLE = "装备属性统计"
elseif (GetLocale() == "zhTW") then
	...
	DUOWAN_GEARSCORELITE = "開啟裝備評分(GS)";
	DUOWAN_ITEMLEVEL_TITLE = "開啟裝備等級顯示";
	GSS_TITLE = "裝備屬性統計"
else
	DUOWAN_CHARACTER_TITLE = "装备信息";
	DUOWAN_CHARACTER_RATING = "开启装备属性合计对比";
...
if (dwIsConfigurableAddOn("TinyTip")) then
	dwRegisterCheckButton(
		"ChracterInfo",
		DUOWAN_ITEMLEVEL_TITLE,
		nil,
		"ItemLevelEnable",
		1,
		function (arg)
			if arg==1 then
				if dwIsAddOnLoaded("TinyTip") and TinyInspectDB then
					TinyInspectDB.EnableItemLevel = true;
				end
			else
				if (dwIsAddOnLoaded("TinyTip") and TinyInspectDB) then	
					TinyInspectDB.EnableItemLevel = false;
				end				
			end
		end
	);	

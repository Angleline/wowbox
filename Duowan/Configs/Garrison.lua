...
if (GetLocale() == "zhCN") then
	...
	HandyNotes_WarfrontRares_ENABLE_TEXT = "战争前线宝箱显示";
...
if dwIsConfigurableAddOn("HandyNotes_WarfrontRares") and dwIsConfigurableAddOn("HandyNotes") then
	dwRegisterCheckButton(
		"Garrison",
		HandyNotes_WarfrontRares_ENABLE_TEXT,
		nil,
		"EnableWarfrontRares",
		0,
		function (arg)
			if ( arg == 1 ) then
				...
				if not dwIsAddOnLoaded("HandyNotes_WarfrontRares") then
					dwLoadAddOn("HandyNotes_WarfrontRares");
					if not HandyNotes.plugins["Arathi"] then
						Arathi:RegisterWithHandyNotes();
					end
				end
			else
				if dwIsAddOnLoaded("HandyNotes_WarfrontRares") then
					dwRequestReloadUI(nil);
				end
			end
		end

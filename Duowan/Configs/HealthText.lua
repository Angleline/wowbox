...
if (GetLocale() == "zhCN") then
	...
	HEALTH_MOD_POSITION_HITPOINT = "显示数字连击点数";
elseif (GetLocale() == "zhTW") then
	...
	HEALTH_MOD_POSITION_HITPOINT = "顯示數字連擊點數";
else
	HEALTH_MOD_TITLE = "简易状态";
	HEALTH_MOD_ENABLE_TEXT = "屏幕中心显示玩家和目标的状态";
	HEALTH_MOD_CASTINGBAR_ENABLE_TEXT = "显示目标施法条";
...
if (dwIsConfigurableAddOn("BlinkHealthText")) then
	...
	if (class == "ROGUE" or class == "DRUID" or class == "MONK") then
		dwRegisterCheckButton(
			"HEALTHTEXT",
			HEALTH_MOD_POSITION_HITPOINT,
			nil,
			"hitpoint",
			1,
			function (arg)

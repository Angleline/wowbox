...
dwStaticPopupDialogs["DUOWAN_COMBUCTOR_RELOADUI"] = {
	text = "确定|cffff7000重置整合背包存档并重载界面|r?",
	button1 = TEXT(OKAY),
	button2 = TEXT(CANCEL),
	OnAccept = function()		
		BrotherBags = nil;
...
if (dwIsConfigurableAddOn("Combuctor")) then
	function Combuctor_ToggleBag(switch)
		...
		Combuctor.profile['inventory'].enabled = switch
	...
	function Combuctor_ToggleBank(switch)
		...
		Combuctor.profile['bank'].enabled = switch
		Combuctor:UpdateFrames()
	end

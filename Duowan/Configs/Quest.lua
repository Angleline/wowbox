...
if dwIsConfigurableAddOn("Wholly") then
	local check = CreateFrame("CheckButton", "QuestHubberLoaderCheckButton", WorldMapFrame.BorderFrame, "InterfaceOptionsBaseCheckButtonTemplate");
	check:SetWidth(28);
	check:SetHeight(28);	
	check:SetPoint("TOPLEFT", WorldMapFrame.ScrollContainer, "TOPLEFT", 5, -5);
	check:SetFrameLevel(check:GetFrameLevel()+2);
	local font, size = GameTooltipHeader:GetFont();
	check.text = check:CreateFontString();
	check.text:SetFont(font, 12, "OUTLINE");

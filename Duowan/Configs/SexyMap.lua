...
if (dwIsConfigurableAddOn("RareScanner")) then
	dwRegisterCheckButton(
		"MiniMapMod",
		...
		function (arg)		
			if (arg == 1) then				
				if (not dwIsAddOnLoaded("RareScanner")) then					
					dwLoadAddOn("RareScanner");
				end
				local RareScanner = LibStub("AceAddon-3.0"):GetAddon("RareScanner")
				RareScanner:Toggle(true);
			else
				if (dwIsAddOnLoaded("RareScanner")) then
					local RareScanner = LibStub("AceAddon-3.0"):GetAddon("RareScanner")
					RareScanner:Toggle(false);
				end
			end
		end
	);
	...
	dwRegisterButton(
		...
		function()
			if (dwIsAddOnLoaded("RareScanner")) then
				local RareScanner = LibStub("AceAddon-3.0"):GetAddon("RareScanner")
				_G.InterfaceOptionsFrame_OpenToCategory(RareScanner.optionsFrame)
				_G.InterfaceOptionsFrame_OpenToCategory(RareScanner.optionsFrame)
			end
		end, 
		1

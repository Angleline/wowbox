...
-- Localization
--==============
local DEFAULT, TITLES, ACE2, AUTHOR, SEPARATE_LOD_LIST, GROUP_BY_NAME, ACP_DUOWAN_ADDON;
local ACP_DUOWAN_ADDON = "Duowan";
...
elseif (GetLocale() == "zhTW") then
	...
	ACE2 = "Ace2"
	AUTHOR = "Author"
	SEPARATE_LOD_LIST = "Separate LOD List"
	GROUP_BY_NAME = "Group By Name"
	ACP_DUOWAN_ADDON= "ACP_DUOWAN_ADDON"
end
...
local ACP_ADDON_NAME = "ACP_DUOWAN_ADDON"
...
----------
-- added by dugu
function ACP:ToggleShowDuowan(switch)
	if (self.Config.showDuowan ~= switch and self.curSortFunc) then
		self.Config.showDuowan = (switch == 1 and true) or false;
		dwSetCVar("ACP", "showDuowan", switch);
		self.curSortFunc();
		self:RebuildSortedAddonList();
		self:FileterDuowanAddOn();
		self:AddonList_OnShow();
	end
end

function ACP_FilterDuowan_OnLoad(self)
	self:RegisterForClicks("LeftButtonUp", "RightButtonUp");
	_G[self:GetName() .. "Text"]:SetFont(ConfigFont:GetFont(),12);	
	_G[self:GetName() .. "Text"]:SetText(L["Show Duowan AddOns"]);
	self.text = _G[self:GetName() .. "Text"];					
end

function ACP_FilterDuowan_OnClick(self)
	local value = self:GetChecked();

	if (value == 1) then
		ACP:ToggleShowDuowan(1);
	else
		ACP:ToggleShowDuowan(0);
	end
end

function ACP_FilterDuowan_OnShow(self)
	if (ACP.Config.showDuowan) then
...
function ACP:HEAD(title)
	return "|CFF0AFF0A" .. tostring(title) .. "|R";
end
...
function ACP:OnLoad(this)
	...
	if (dwRawGetCVar("ACP", "showDuowan", 0) == 1) then
		self.Config.showDuowan = true;
	else
		self.Config.showDuowan = false;
	end
	...
	dwStaticPopupDialogs["ACP_RELOADUI"] = {
		...
		OnCancel = function(data, reason)
    		if ( reason == "timeout" ) then
    		   ...
    		   dwStaticPopupDialogs["ACP_RELOADUI"].reloadAccepted = false
    		end
    	end,
    	OnHide = function()
    		if (dwStaticPopupDialogs["ACP_RELOADUI"].reloadAccepted ) then
    			ReloadUI();
    		end
    	end,
		OnShow = function()
    	    dwStaticPopupDialogs["ACP_RELOADUI"].reloadAccepted = true;
    	end,
		timeout = 5,
		hideOnEscape = 1,
		exclusive = 1,
	...
	dwStaticPopupDialogs["ACP_RELOADUI_START"] = {
	...
	dwStaticPopupDialogs["ACP_SAVESET"] = {
	...
	dwStaticPopupDialogs["ACP_RENAMESET"] = {
...
function ACP:OnEvent(this, event, arg1, arg2, arg3)
	if event == "VARIABLES_LOADED" then
		if not DuowanAddon_ACP_Data then DuowanAddon_ACP_Data = {} end

		savedVar = DuowanAddon_ACP_Data
		...
		if not savedVar.sorter then
			ACP:SetMasterAddonBuilder(ACP_DUOWAN_ADDON)
		else
			ACP:ReloadAddonList()
		end
	...
	elseif event == "PLAYER_ALIVE" then
		...
		if savedVar.reloadRequired then
		    dwStaticPopup_Show("ACP_RELOADUI_START");
		end
...
addonListBuilders[ACP_DUOWAN_ADDON] = function()
	local tmp, t = {}, {};
	...
	-- Sort the addon list by DuowanAddon Categories.
	table.sort(tmp, function(a, b)
		local catA = GetAddOnMetadata(a, "X-DuowanAddon");
		local catB = GetAddOnMetadata(b, "X-DuowanAddon");
		if catA == catB then
	...
	local prevCategory = ""
	for i, addonIndex in ipairs(tmp) do
		local category = GetAddOnMetadata(addonIndex, "X-DuowanAddon")
		if not category then
			category = L["Undefined"];
		end
		if category ~= prevCategory then
			table.insert(tmp, i, category);
		end
		prevCategory = category;
	end

	table.insert(tmp, L["Blizzard"]);

	for i=1, #ACP_BLIZZARD_ADDONS do
		table.insert(tmp, numAddons+i);
	end	
	-- Now build the masterAddonList.
	for k in pairs(masterAddonList) do
		masterAddonList[k] = nil
...
function ACP:EnableAddon(addon, shift, ctrl)
	if (DuowanAddon_ACP_Data) then
		local norecurse = DuowanAddon_ACP_Data.NoRecurse
		if shift then norecurse = not norecurse end

		local nochildren = DuowanAddon_ACP_Data.NoChildren
...
function ACPEnableAddon(addon)
	ACP:EnableAddon(addon,0,0);
end
...
function ACP:Security_OnClick(addon)
     ...
     if addon then
         savedVar.ProtectedAddons = savedVar.ProtectedAddons or { ["ACP_DUOWAN_ADDON"] = true }
...
do
	...
	function ACP:FileterDuowanAddOn()
		if (not self.Config.showDuowan) then
			local tmp = clone(sortedAddonList);
			for k in pairs(sortedAddonList) do
				sortedAddonList[k] = nil
			end
			local name, isDuowanAddOn, Revision;
			local addonNum = GetNumAddOns();
			for k, addon in ipairs(tmp) do
				if (type(addon) == "number") then
					if (addon < addonNum) then						
						name = GetAddOnInfo(addon);
						Revision  = GetAddOnMetadata(name, "X-Revision");
						
						if (not Revision or Revision ~= "Duowan") then								
...
--ACP_AddonListFilterDuowanButton
	...
	elseif level == 2 then
		...
		if L_UIDROPDOWNMENU_MENU_VALUE ~= ACP_DEFAULT_SET then
			...
			info.func = function()
				...
				dwStaticPopup_Show("ACP_SAVESET", setName)
			end
			info.notCheckable = 1
			L_UIDropDownMenu_AddButton(info, level)
		end
		...
		if L_UIDROPDOWNMENU_MENU_VALUE ~= ACP_DEFAULT_SET and L_UIDROPDOWNMENU_MENU_VALUE ~= playerClass then
			...
			info.func = function()
				...
				dwStaticPopup_Show("ACP_RENAMESET", setName)
				CloseDropDownMenus(1)
			end
			info.notCheckable = 1
			L_UIDropDownMenu_AddButton(info, level)

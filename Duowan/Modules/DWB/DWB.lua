...
local function UpdatePositionValue()
	...
	elseif ( StanceBarFrame and StanceBarFrame:IsShown() and GetNumShapeshiftForms() > 0 ) then
...
function P:CreatePetActionBar()
	...
	RegisterStateDriver(self.bar, "visibility", "[nopet]hide;[target=vehicle,exists]hide;[petbattle]hide;[vehicleui]hide;show");
...
function P:CreateLeaverVehicleButton()
	if (not self.leavebtn) then
		self.leavebtn = CreateFrame("Button", "dwVehicleLeaveButton", UIParent, "SecureHandlerClickTemplate")
	...
	--	RegisterStateDriver(self.leavebtn, "visibility", "[vehicleui][target=vehicle,noexists] hide;show")

		self.leavebtn:SetScript("OnClick", function(self)
		...
		hooksecurefunc("MainMenuBarVehicleLeaveButton_Update", function()
			if (CanExitVehicle() and ActionBarController_GetCurrentActionBarState() == LE_ACTIONBAR_STATE_MAIN) then
				...
				elseif ( GetNumShapeshiftForms() > 0 ) then
					self.leavebtn:SetPoint("LEFT", "StanceButton"..GetNumShapeshiftForms(), "RIGHT", 30, 0);

...
if not AlwaysUpFrame1 then
	AlwaysUpFrame1 = CreateFrame("Frame", "AlwaysUpFrame1", UIParent)
	AlwaysUpFrame2 = CreateFrame("Frame", "AlwaysUpFrame2", UIParent)
end
...
function BattleInfo_Server:Update()
	...
	if AlwaysUpFrame1Text and AlwaysUpFrame2Text then
		...
		for i=1,80 do
			...
			local name, _, _, _, _, faction, _, _, class, filename, _, _ = GetBattlefieldScore(i)
			if name and faction and filename and self.data[faction] then
				...
				self.data[faction].class[class]=self.data[faction].class[class]+1
			end	

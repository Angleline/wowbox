...
--print('GridStatusRaidDebuff:OnInitialize')
    if not self.db then
		self.db = Grid.db:RegisterNamespace(self.menuName, { profile = self.defaultDB or { } })
	end
	if not self.options then
		self.options = {
			name = self.menuName or self.moduleName,
			desc = format(L["Options for %s."], self.menuName),
			type = "group",
			args = {},
		}
	end

	...
	if ( switch == 1 ) then 
		if (not NPCMark_Loaded) then
			for pin in WorldMapFrame:EnumeratePinsByTemplate("MapExplorationPinTemplate") do
				hooksecurefunc(pin, "RefreshOverlays", function(pin)
					NPCMark_WorldMapFrameOnUpdate(pin)
				end)
			end
...
function NPCMark_WorldMapFrameOnShow()
	...
	if not MapMarkCheckButton.reset then
		MapMarkCheckButton:ClearAllPoints();
		MapMarkCheckButton:SetPoint("TOPRIGHT", WorldMapFrame.ScrollContainer, "TOPRIGHT", -140, -5)
		MapMarkCheckButton:SetParent(WorldMapFrame.ScrollContainer)
		MapMarkCheckButton.reset = true
		MapPlusDDList:SetScale(0.7)
		MapPlusDDList:SetParent(MapMarkCheckButton)
	end
end 

local function isSelected(type) 
	if NPCMarkDB and NPCMarkDB[GetMappedType(type)] then
		return true
	end 
...
local function showNodes(_type,name,...)
	local function showNode(name,_type,x,y)
		...
		if not button then
			button = CreateFrame("Button","MapMark"..node_index,WorldMapFrame.ScrollContainer,"MapMarkTemplate") 
		end
		button:SetPoint("CENTER",WorldMapFrame.ScrollContainer,"TOPLEFT",coord_transform(WorldMapFrame.ScrollContainer:GetWidth(), WorldMapFrame.ScrollContainer:GetHeight(),x,y))
...
local function getCurrentMapName(pin) 
	...
	--	if GetCurrentMapContinent() == 7 then return; end --skip WOD Zone
		...
		return HereBeDragons:GetLocalizedMap(mapId), currLevel; 
	end
end
...
function NPCMark_WorldMapFrameOnUpdate(pin) 
	hideNodes() 
	if not NPCMark_Enable then 
		return 
	end
	...
	local mapName, currLevel = getCurrentMapName(pin) 

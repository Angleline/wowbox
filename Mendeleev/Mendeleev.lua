...
function Mendeleev:OnInitialize()
	...
	if (self.db.profile.showHuahuaSet == nil) then
		self.db.profile.showHuahuaSet = true;
	end
	if (self.db.profile.showsetdressup == nil) then
		self.db.profile.showsetdressup = true;
	end
...
function Mendeleev:OnEnable()
	...
	self:SecureHook(GameTooltip, "SetRecipeReagentItem")
	...
	self:RegisterEvent("TRADE_SKILL_LIST_UPDATE", "ScanTradeSkill")
end

-- function Mendeleev:OnDisable()
-- end

function Mendeleev:DressUpItemLink(link)
	if (self.db.profile.showsetdressup and IsAltKeyDown()) then
	...
	if PT:IsSetMulti("TradeskillResultMats.Reverse." .. skill) then
		for k, v in ipairs(ptrmr) do
			if type(v) == "table" then
				local subskill = v.set:match("TradeskillResultMats%.Reverse%.(.+)")
				if subskill then
	...
	for i = 4, #tree do
		...
		if v[3] >= UsedInTreeMinSkill then
			if UsedInTreeIcons then
				...
				icontag = icontag and "|T"..icontag..":14|t " or ""
...
function Mendeleev:ScanTradeSkill()
	if (C_TradeSkillUI.IsTradeSkillLinked() or C_TradeSkillUI.IsTradeSkillGuild() or C_TradeSkillUI.IsNPCCrafting()) then
		-- don't scan other persons or guild
		return
	end
	local allRecipeIDs = C_TradeSkillUI.GetAllRecipeIDs()
	for i, recipeID in ipairs(allRecipeIDs) do
		local recipeInfo = C_TradeSkillUI.GetRecipeInfo(recipeID)
		if recipeInfo.type == "recipe" and recipeInfo.learned then
			local item = C_TradeSkillUI.GetRecipeItemLink(recipeInfo.recipeID)
			if item then
				local id = tonumber(item:match("item:(%d+)")) or -tonumber(item:match("enchant:(%d+)"))
				if id then
					id2skill[id] = skillquals[recipeInfo.difficulty]
				end
			end
		end
	end
...
function Mendeleev:GetSetItemCount(sets)
	local count = 0;
...
function Mendeleev:OnTooltipSetItem(tooltip, ...)
	...
	self:AddTooltipData(tooltip, item)
		...
		-----------------
	-- 显示幻化套装信息
	if (self.db.profile.showHuahuaSet) then
		local iType, subType, setName, setsdata = huanhuadata:FindItem(itemID);		
		if (iType) then
			local curCount, totalCount = self:GetSetItemCount(setsdata);
			local text = "\n[幻化] ";
			...
			if (self.db.profile.showsetdressup) then				
				tooltip:AddLine("\nCtrl + Alt + Left键可查看套装效果", 0, 0.6, 0.8);
			end
		end
	end
end

function Mendeleev:SetRecipeReagentItem(tooltip, recipe, reagent)
	local item = C_TradeSkillUI.GetRecipeReagentItemLink(recipe, reagent)
	if tooltip.Mendeleev_data_added or not item or not GetItemInfo(item) then return end
	self:AddTooltipData(tooltip, item)
	tooltip:Show()
end
...
function Mendeleev:AddTooltipData(tooltip, item)
	...
	local LI = LibStub("LibInventory-2.1", true)
	if (LI) then
		if (LI:IsClientItem(item)) then
			tooltip:AddDoubleLine("data source:", "client")
		else
			tooltip:AddDoubleLine("data source:", "server")
		end
	end
	...
	if db.showUsedInTree and not InCombatLockdown() then
...
function Mendeleev_ShowUsedInTree(switch)
	if (switch) then
		Mendeleev.db.profile.UsedInTreeIcons = true;
	else
		Mendeleev.db.profile.UsedInTreeIcons = false;
	end
end

function Mendeleev_ShowSetsInfo(switch)
	if (switch) then
		Mendeleev.db.profile.showHuahuaSet = true;
	else
		Mendeleev.db.profile.showHuahuaSet = false;
	end
end

function Mendeleev_DressUpSets(switch)
	if (switch) then
		Mendeleev.db.profile.showsetdressup = true;
	else
		Mendeleev.db.profile.showsetdressup = false;
	end
end

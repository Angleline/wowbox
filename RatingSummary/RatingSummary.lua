...
function RatingSummary_OnEvent(self, event, ...)
	...
	if (event == "INSPECT_HONOR_UPDATE") then
		...
		for i = 1, MAX_ARENA_TEAMS do
			...
			if teamSize == 2 then
				frame.text1:SetFormattedText("%s |cffFFFFFF%s|r/|cff666666%s|r", "本赛季：", seasonWon, seasonPlayed)
			elseif teamSize == 3 then
				frame.text2:SetFormattedText("%s |cffFFFFFF%s|r/|cff666666%s|r", "本周：", weeklyWon, weeklyPlayed)
			elseif teamSize == 5 then
				frame.text3:SetFormattedText("%s |cffFFFFFF%s|r", "个人等级：", arenarating)
			end
		end
	end
...
function RatingSummary_GetTalentString(isInspecting, idx) 
	...
	for v = 1,3 do
		local id, tn, description, iconTexture, num, previewPointsSpent, isUnlocked = GetSpecializationInfo(v, isInspecting, false, idx)
		if num > max then
			max = num;
			main = tn;
		end

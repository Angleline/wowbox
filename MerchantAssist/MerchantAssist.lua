...
local _DEBUG = false;
local ident = GetRealmName() .. "_" .. playerName;
...
dwStaticPopupDialogs["MERCHANT_AUTOSELL_CONFIRM"] = {
...
dwStaticPopupDialogs["MERCHANT_DELETE_AUTOSELL"] = {
	text = L["确定从自动售卖列表中取消\n%s"],
	button1 = TEXT(OKAY),
	button2 = TEXT(CANCEL),
	OnAccept = function()
...
dwStaticPopupDialogs["MERCHANT_AUTOBUY_CONFIRM"] = {
	text = L["确定将 %s 设置为自动购买物品"],
	button1 = TEXT(OKAY),
	button2 = TEXT(CANCEL),
	OnAccept = function()
		if (GetItemInfo(dwStaticPopupDialogs["MERCHANT_AUTOBUY_CONFIRM"].ItemID)) then
			MerchantAssist:AddToList(dwStaticPopupDialogs["MERCHANT_AUTOBUY_CONFIRM"].ItemID);
...
dwStaticPopupDialogs["MERCHANT_DELETE_AUTOBUY"] = {
	text = L["确定取消对%s的自动购买"],
	button1 = TEXT(OKAY),
	button2 = TEXT(CANCEL),
	OnAccept = function()
...
dwStaticPopupDialogs["MERCHANT_AUTOBUY_ITEM_CONFIRM"] = {
	text = L["确定购买以下物品:\n %s"],
	button1 = TEXT(OKAY),
	button2 = TEXT(CANCEL),
	OnAccept = function()
		if (dwStaticPopupDialogs["MERCHANT_AUTOBUY_ITEM_CONFIRM"].list and MerchantAssist:getn(dwStaticPopupDialogs["MERCHANT_AUTOBUY_ITEM_CONFIRM"].list)) then
			local cost = 0;
			for id, need in pairs(dwStaticPopupDialogs["MERCHANT_AUTOBUY_ITEM_CONFIRM"].list) do				
...
function MerchantAssist:BuyItems()
	...
	for id, v in pairs(self.db.autobuy) do
		if (v.show and GetItemInfo(id) and (type(self["ClassItemInfo"][id]) ~= "function" or self["ClassItemInfo"][id]())) then
	...
	if (self:getn(tmp) > 0) then
		dwStaticPopupDialogs["MERCHANT_AUTOBUY_ITEM_CONFIRM"].list = tmp;
		dwStaticPopup_Show("MERCHANT_AUTOBUY_ITEM_CONFIRM", text);
	end
end
...
--	local itemID;
	...
	--	if (string.find(link, "Hbattlepet:")) then return -1 end
	--	if (string.find(link, "keystone:")) then return -1 end
...
function MerChantBaseItem_OnClick(self, button)
		...
		if (IsControlKeyDown()) then
			...
			if (link) then
				if (MerchantAssist.db.autosell[curAutoSellItemID]) then					
					dwStaticPopup_Show("MERCHANT_DELETE_AUTOSELL", link);
				else
					dwStaticPopup_Show("MERCHANT_AUTOSELL_CONFIRM", link);
				end
			end
		end
	elseif (MerchantAssist.curType == "autobuy") then
		...
		if (link) then
			...
			dwStaticPopup_Show("MERCHANT_DELETE_AUTOBUY", link);
		end
	end
end
...
function MerchantAssist:IsItemAutoSell(itemID, itemName, quality)
	if ((itemID and self.db["autosell"][itemID] or (quality and quality == 0)) and not self:IsIgnorItem(itemName)) then
		return true;
	end
	return false;
end
...
function MerchantAutoBuyAddItemButton_OnClick(self, button)	
	...
	if (link) then	
		...
		dwStaticPopupDialogs["MERCHANT_AUTOBUY_CONFIRM"].ItemID = MerchantAssist:GetItemID(link);
		dwStaticPopup_Show("MERCHANT_AUTOBUY_CONFIRM", link);
	end
end

...
do
	...
	local function creategui()
		...
		do
			local area = panel:CreateArea(L.AreaGeneral, nil, 210, true)
			...
			AOEbyGuildRank:SetPoint("TOPLEFT", area.frame, "TOPLEFT", -3, -175)


			local aoeinv = area:CreateButton(L.Button_AOE_Invite, 120)
			aoeinv:SetPoint("TOPLEFT", AOEbyGuildRank, "TOPRIGHT", 40, -5)
...
--			keyword:SetScript("OnShow", 		function(self) self:SetText(settings.keyword) end)
		...
		do
			...
			PromoteGuildRank:SetPoint("TOPLEFT", area.frame, "TOPLEFT", 0, -55)
			...
			PromoteByNameList:SetPoint("TOPLEFT", area.frame, "TOPLEFT", 27, -100)

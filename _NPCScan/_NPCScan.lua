...
function private.SetAlertSound(alert_sound)
	...
	_G.Lib_UIDropDownMenu_SetText(private.Config.alert_sound_dropdown, alert_sound == nil and L.CONFIG_ALERT_SOUND_DEFAULT or alert_sound)
end
...
function private.SetTargetIcon(icon_id)
	...
	_G.Lib_UIDropDownMenu_SetText(private.Config.alert_icon_dropdown, colorCode .. icon_info.text)
end
...
local function GetEnabled ( Name ) 
	...
	return _NPCScan.CharacterOptions.EnableScan and Options.Modules[ Name ] ~= false;
end

local modules = {"Minimap", "BattlefieldMinimap", "WorldMap"};
function _NPCScan:Toggle(switch)
	if (switch) then
		private.CharacterOptions.EnableScan = true;
		_NPCScanOverlayWorldMapToggle:Show();
		--_NPCScanOverlayWorldMapToggle:Click("LeftButton");
	else
		private.CharacterOptions.EnableScan = false;
		_NPCScanOverlayWorldMapToggle:Hide();
	end
...
function _NPCScan:Minimap_Toggle(switch)
	if (switch) then
		...
		private.Overlay.Modules["Enable"]("Minimap");
	else
		--_NPCScanOverlayOptions.Modules["Minimap"] = false;
		private.Overlay.Modules["Disable"]("Minimap");
	end
end
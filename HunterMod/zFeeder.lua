...
function z:CreateButton()
	if (not self.button) then
		self.button = CreateFrame("ItemButton", "zFeederButton", PetFrame, "SecureActionButtonTemplate");
...
function z:ParseItemLink(link)
	if (link) then
		for id, name in string.gmatch(link, "|c%x+|Hitem:(%d+):.+|h%[(.+)%]|h|r") do			
...
function z:HasFeedEffect()
	local i = 1;	
	local name, texture, count, debuffType, duration, expirationTime = UnitAura("pet", i);
	...
	while (texture) do
		...
		i = i + 1;
		name, texture, count, debuffType, duration, expirationTime = UnitAura("pet", i);
	end
	return nil;
end

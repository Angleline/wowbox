...
function MYSLOT_ShowTitanButton(switch)
	if (not TitanPanelSettings) then
		dwDelayCall("TitanPanelShowCustomButton", 2, "LDBT_MYSLOT", switch);
	else
		TitanPanelShowCustomButton("LDBT_MYSLOT", switch);
	end	
end

function MYSLOT_RegisterTitan_Button()
	LibStub("LibDataBroker-1.1"):NewDataObject("MYSLOT", {
		icon = "Interface\\ICONS\\Ability_Racial_TimeIsMoney",
		label = "|cFF33FF33"..MySlot_TitleText.."|r",
		type = "launcher",
		text  = MySlot_TitleText,
		OnClick = function(clickedFrame, button)
			ShowUIPanel(MYSLOT_ReportFrame);
		end,
		OnTooltipShow = function(tt)
			tt:SetText(MySlot_TitleText, 1, 1, 1);
			tt:AddLine(MySlot_TitleTip);
		end,
	});	
end
